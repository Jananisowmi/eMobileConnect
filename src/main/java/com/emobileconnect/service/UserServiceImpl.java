...
public class UserServiceImpl implements UserService {
	 ...
	 */
	@Override
	public UserRegistrationResponseDto register(UserRegistrationRequestDto userRequestDto) throws UserRegistrationException {
		...
		Optional<TalktimePlan> checkTalktimePlan = talktimePlanRepository.findByTalktimePlanId(userRequestDto.getPlanId());		
		if(!checkTalktimePlan.isPresent()) {
			throw new UserRegistrationException(MobileConnectionConstants.NO_PLAN_EXISTS);
		}
		
		Optional<MobileNumber> checkAvailableNumber = mobileNumberRepository.findByMobileNumber(userRequestDto.getMobileNumber());		
